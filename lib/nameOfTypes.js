const NameOfTypes = {
    ...
    tankLevel: { label: 'Tank level', description: 'Fill level of a tank (read-only). Value in `%` or in an absolute unit like liters; if `min`/`max` (or only `max`) are defined, both units can be calculated.' },
