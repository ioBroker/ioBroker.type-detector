...
writeLine('## Devices');
writeLine('In [brackets] is given the class name of device.');

writeLine('### Content');
Object.keys(patterns).sort().forEach(type => {
    if (patterns[type] && type !== 'instance') {
        const name = Names[patterns[type].type] || type;
...
Object.keys(patterns).sort().forEach(type => {
    if (type !== 'instance') {
        writeLine(`### ${Names[patterns[type].type] || type}${Names[patterns[type].type] ? ` [${type}]` : ''}`);
        const lines = [];
...
Object.keys(enums).forEach(enu => {
    writeLine(`### ${enu}`);
    writeLine('To detect these devices, it must belong to one of the following categories (any regex in any language):');
    Object.keys(enums[enu].words).forEach(language => {
        writeLine(`- **${language}**: ${enums[enu].words[language].map(r => '``' + r.toString().replace(/\|/g, '\|') + '``').join(', ')}`);
    });
