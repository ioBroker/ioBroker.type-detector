...
import ChannelDetectorImport from '../build/esm/index.js';
const ChannelDetector = ChannelDetectorImport;
import Names from './nameOfTypes.js';
...
writeLine('');
writeLine('While many devices have similar states (e.g., only one boolean state like by a door and window sensors), the following categories (enumerations) and roles are used to distinguish one type from another.');
writeLine('E.g., if state has role `"door"` and type `boolean`, it is a door sensor. If it has role `"window"` and type `boolean`, it is a window sensor.');
writeLine('If the role is not provided, the type detector tries to use categories to detect the type. The state or the parent channel/device could belong to one of the categories, so the type detector can decide what the device is that.');
writeLine('First, the role will be used for detection and then the categories.');
writeLine('');

const enums = ChannelDetector.getEnums();
Object.keys(enums).forEach(enu => {
    writeLine(`### ${Names[enu].label} [${enu}]`);
    writeLine(
    ...
    Object.keys(enums[enu].words).forEach(language => {
        writeLine(
            `- **${language}**: ${enums[enu].words[language].map(r => `\`${r.toString()}\``).join(', ')}`,
        );
    });
