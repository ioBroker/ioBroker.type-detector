...
function createTests(name, ChannelDetector, Types) {
    describe(`${name} Test Detector`, () => {
        it(`${name} Must detect temperature sensor from channel`, done => {
        ...
        it('Must detect cameras from states', done => {
            ...
            const powerId = controls[0].states.find(s => s.name === 'URL').id;
        ...
        it('Must detect forecast from accuweather and assign days correctly', done => {
            ...
            for (const day of days) {
                expectMyStateToHaveId(
                    `DATE${day}`,
                    `accuweather.0.Summary.DateTime_d${day + 1}`
                );
                expectMyStateToHaveId(
                    `ICON${day}`,
                    `accuweather.0.Summary.WeatherIconURL_d${day + 1}`
                );
                expectMyStateToHaveId(
                    `STATE${day}`,
                    `accuweather.0.Summary.WeatherText_d${day + 1}`
                );
                expectMyStateToHaveId(
                    `TEMP_MIN${day}`,
                    `accuweather.0.Summary.TempMin_d${day + 1}`
                );
                expectMyStateToHaveId(
                    `TEMP_MAX${day}`,
                    `accuweather.0.Summary.TempMax_d${day + 1}`
                );
                expectMyStateToHaveId(
                    `WIND_SPEED${day}`,
                    `accuweather.0.Summary.WindSpeed_d${day + 1}`
                );
                expectMyStateToHaveId(
                    `WIND_DIRECTION${day}`,
                    `accuweather.0.Summary.WindDirection_d${day + 1}`
                );
                expectMyStateToHaveId(
                    `WIND_DIRECTION_STR${day}`,
                    `accuweather.0.Summary.WindDirectionStr_d${day + 1}`
                );
                expectMyStateToHaveId(
                    `DOW${day}`,
                    `accuweather.0.Summary.DayOfWeek_d${day + 1}`
                );
                expectMyStateToHaveId(
                    `PRECIPITATION_CHANCE${day}`,
        ...
        it('Must detect forecast from dasWetter and assign days correctly', done => {
            ...
            for (const day of days) {
                expectMyStateToHaveId(
                    `ICON${day}`,
                    `daswetter.0.NextDays.Location_1.Day_${day + 1}.iconURL`
                );
                expectMyStateToHaveId(
                    `TEMP_MIN${day}`,
                    `daswetter.0.NextDays.Location_1.Day_${day + 1}.Minimale_Temperatur_value`
                );
                expectMyStateToHaveId(
                    `TEMP_MAX${day}`,
                    `daswetter.0.NextDays.Location_1.Day_${day + 1}.Maximale_Temperatur_value`
                );
                expectMyStateToHaveId(
                    `DOW${day}`,
                    `daswetter.0.NextDays.Location_1.Day_${day + 1}.Tag_value`
                );
            }
            done();
        });
